...
setCustomSpacing_afterView nsStackView  spacing view =
  withObjCPtr view $ \raw_view ->
      sendMsg nsStackView (mkSelector "setCustomSpacing:afterView:") retVoid [argCDouble spacing, argPtr (castPtr raw_view :: Ptr ())]
...
insertArrangedSubview_atIndex nsStackView  view index =
  withObjCPtr view $ \raw_view ->
      sendMsg nsStackView (mkSelector "insertArrangedSubview:atIndex:") retVoid [argPtr (castPtr raw_view :: Ptr ()), argCLong index]
...
setVisibilityPriority_forView nsStackView  priority view =
  withObjCPtr view $ \raw_view ->
      sendMsg nsStackView (mkSelector "setVisibilityPriority:forView:") retVoid [argCFloat priority, argPtr (castPtr raw_view :: Ptr ())]
...
setClippingResistancePriority_forOrientation nsStackView  clippingResistancePriority orientation =
    sendMsg nsStackView (mkSelector "setClippingResistancePriority:forOrientation:") retVoid [argCFloat clippingResistancePriority, argCLong (coerce orientation)]
...
setHuggingPriority_forOrientation nsStackView  huggingPriority orientation =
    sendMsg nsStackView (mkSelector "setHuggingPriority:forOrientation:") retVoid [argCFloat huggingPriority, argCLong (coerce orientation)]
...
insertView_atIndex_inGravity nsStackView  view index gravity =
  withObjCPtr view $ \raw_view ->
      sendMsg nsStackView (mkSelector "insertView:atIndex:inGravity:") retVoid [argPtr (castPtr raw_view :: Ptr ()), argCULong index, argCLong (coerce gravity)]
...
setSpacing nsStackView  value =
    sendMsg nsStackView (mkSelector "setSpacing:") retVoid [argCDouble value]
