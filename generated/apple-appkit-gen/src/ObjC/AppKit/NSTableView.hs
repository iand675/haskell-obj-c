...
moveColumn_toColumn nsTableView  oldIndex newIndex =
    sendMsg nsTableView (mkSelector "moveColumn:toColumn:") retVoid [argCLong oldIndex, argCLong newIndex]
...
scrollRowToVisible nsTableView  row =
    sendMsg nsTableView (mkSelector "scrollRowToVisible:") retVoid [argCLong row]

-- | @- scrollColumnToVisible:@
scrollColumnToVisible :: IsNSTableView nsTableView => nsTableView -> CLong -> IO ()
scrollColumnToVisible nsTableView  column =
    sendMsg nsTableView (mkSelector "scrollColumnToVisible:") retVoid [argCLong column]
...
setDropRow_dropOperation nsTableView  row dropOperation =
    sendMsg nsTableView (mkSelector "setDropRow:dropOperation:") retVoid [argCLong row, argCULong (coerce dropOperation)]
...
frameOfCellAtColumn_row nsTableView  column row =
    sendMsgStret nsTableView (mkSelector "frameOfCellAtColumn:row:") retNSRect [argCLong column, argCLong row]
...
editColumn_row_withEvent_select nsTableView  column row event select =
  withObjCPtr event $ \raw_event ->
      sendMsg nsTableView (mkSelector "editColumn:row:withEvent:select:") retVoid [argCLong column, argCLong row, argPtr (castPtr raw_event :: Ptr ()), argCULong (if select then 1 else 0)]

-- | @- drawRow:clipRect:@
drawRow_clipRect :: IsNSTableView nsTableView => nsTableView -> CLong -> NSRect -> IO ()
drawRow_clipRect nsTableView  row clipRect =
    sendMsg nsTableView (mkSelector "drawRow:clipRect:") retVoid [argCLong row, argNSRect clipRect]
...
viewAtColumn_row_makeIfNecessary nsTableView  column row makeIfNecessary =
    sendMsg nsTableView (mkSelector "viewAtColumn:row:makeIfNecessary:") (retPtr retVoid) [argCLong column, argCLong row, argCULong (if makeIfNecessary then 1 else 0)] >>= retainedObject . castPtr
...
rowViewAtRow_makeIfNecessary nsTableView  row makeIfNecessary =
    sendMsg nsTableView (mkSelector "rowViewAtRow:makeIfNecessary:") (retPtr retVoid) [argCLong row, argCULong (if makeIfNecessary then 1 else 0)] >>= retainedObject . castPtr
...
moveRowAtIndex_toIndex nsTableView  oldIndex newIndex =
    sendMsg nsTableView (mkSelector "moveRowAtIndex:toIndex:") retVoid [argCLong oldIndex, argCLong newIndex]
...
didAddRowView_forRow nsTableView  rowView row =
  withObjCPtr rowView $ \raw_rowView ->
      sendMsg nsTableView (mkSelector "didAddRowView:forRow:") retVoid [argPtr (castPtr raw_rowView :: Ptr ()), argCLong row]
...
didRemoveRowView_forRow nsTableView  rowView row =
  withObjCPtr rowView $ \raw_rowView ->
      sendMsg nsTableView (mkSelector "didRemoveRowView:forRow:") retVoid [argPtr (castPtr raw_rowView :: Ptr ()), argCLong row]
...
selectColumn_byExtendingSelection nsTableView  column extend =
    sendMsg nsTableView (mkSelector "selectColumn:byExtendingSelection:") retVoid [argCLong column, argCULong (if extend then 1 else 0)]
...
selectRow_byExtendingSelection nsTableView  row extend =
    sendMsg nsTableView (mkSelector "selectRow:byExtendingSelection:") retVoid [argCLong row, argCULong (if extend then 1 else 0)]
...
preparedCellAtColumn_row nsTableView  column row =
    sendMsg nsTableView (mkSelector "preparedCellAtColumn:row:") (retPtr retVoid) [argCLong column, argCLong row] >>= retainedObject . castPtr
...
shouldFocusCell_atColumn_row nsTableView  cell column row =
  withObjCPtr cell $ \raw_cell ->
      fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsTableView (mkSelector "shouldFocusCell:atColumn:row:") retCULong [argPtr (castPtr raw_cell :: Ptr ()), argCLong column, argCLong row]
...
setFocusedColumn nsTableView  focusedColumn =
    sendMsg nsTableView (mkSelector "setFocusedColumn:") retVoid [argCLong focusedColumn]
...
setRowHeight nsTableView  value =
    sendMsg nsTableView (mkSelector "setRowHeight:") retVoid [argCDouble value]
