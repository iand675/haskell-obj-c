...
rulerView_willMoveMarker_toLocation nsTextView  ruler marker location =
  withObjCPtr ruler $ \raw_ruler ->
    withObjCPtr marker $ \raw_marker ->
        sendMsg nsTextView (mkSelector "rulerView:willMoveMarker:toLocation:") retCDouble [argPtr (castPtr raw_ruler :: Ptr ()), argPtr (castPtr raw_marker :: Ptr ()), argCDouble location]
...
rulerView_willAddMarker_atLocation nsTextView  ruler marker location =
  withObjCPtr ruler $ \raw_ruler ->
    withObjCPtr marker $ \raw_marker ->
        sendMsg nsTextView (mkSelector "rulerView:willAddMarker:atLocation:") retCDouble [argPtr (castPtr raw_ruler :: Ptr ()), argPtr (castPtr raw_marker :: Ptr ()), argCDouble location]
...
clickedOnLink_atIndex nsTextView  link charIndex =
    sendMsg nsTextView (mkSelector "clickedOnLink:atIndex:") retVoid [argPtr (castPtr (unRawId link) :: Ptr ()), argCULong charIndex]
...
checkTextInRange_types_options nsTextView  range checkingTypes options =
  withObjCPtr options $ \raw_options ->
      sendMsg nsTextView (mkSelector "checkTextInRange:types:options:") retVoid [argNSRange range, argCULong checkingTypes, argPtr (castPtr raw_options :: Ptr ())]
  ...
  withObjCPtr results $ \raw_results ->
    withObjCPtr options $ \raw_options ->
      withObjCPtr orthography $ \raw_orthography ->
          sendMsg nsTextView (mkSelector "handleTextCheckingResults:forRange:types:options:orthography:wordCount:") retVoid [argPtr (castPtr raw_results :: Ptr ()), argNSRange range, argCULong checkingTypes, argPtr (castPtr raw_options :: Ptr ()), argPtr (castPtr raw_orthography :: Ptr ()), argCLong wordCount]
...
setSpellingState_range nsTextView  value charRange =
    sendMsg nsTextView (mkSelector "setSpellingState:range:") retVoid [argCLong value, argNSRange charRange]
...
insertCompletion_forPartialWordRange_movement_isFinal nsTextView  word charRange movement flag =
  withObjCPtr word $ \raw_word ->
      sendMsg nsTextView (mkSelector "insertCompletion:forPartialWordRange:movement:isFinal:") retVoid [argPtr (castPtr raw_word :: Ptr ()), argNSRange charRange, argCLong movement, argCULong (if flag then 1 else 0)]
...
setEnabledTextCheckingTypes nsTextView  value =
    sendMsg nsTextView (mkSelector "setEnabledTextCheckingTypes:") retVoid [argCULong value]
