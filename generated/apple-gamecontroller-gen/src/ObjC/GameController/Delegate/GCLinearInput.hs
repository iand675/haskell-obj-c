...
-- | Generated delegate overrides for @\@protocol GCLinearInput@.
--
-- Usage:
--
-- @
-- delegate <- newGCLinearInput defaultGCLinearInputOverrides
--   { ... = Just $ \\arg0 -> ...
--   }
-- @
module ObjC.GameController.Delegate.GCLinearInput
  ( GCLinearInputOverrides(..)
  , defaultGCLinearInputOverrides
  , newGCLinearInput
  ) where
...
-- | Overrides record for @\@protocol GCLinearInput@.
--
-- Each field corresponds to a protocol method.  'Nothing' means the
-- method is not implemented (the object will not respond to that
-- selector).  'Just' provides the Haskell implementation.
data GCLinearInputOverrides = GCLinearInputOverrides
  ...
  , _physicalExtents :: !(Maybe (IO RawId))
  , _sources :: !(Maybe (IO RawId))
  }

-- | Default overrides with all methods unimplemented.
defaultGCLinearInputOverrides :: GCLinearInputOverrides
defaultGCLinearInputOverrides = GCLinearInputOverrides
...
{-# NOINLINE gcLinearInputDelegateClass #-}
gcLinearInputDelegateClass :: Class
gcLinearInputDelegateClass = unsafePerformIO $ do
  superCls <- getRequiredClass "NSObject"
  cls <- withCString "HsGCLinearInput" $ \n ->
  ...
  let sel_valueDidChangeHandler = unSelector (mkSelector "valueDidChangeHandler")
      ...
      sel_physicalExtents = unSelector (mkSelector "physicalExtents")
      sel_sources = unSelector (mkSelector "sources")
  -- valueDidChangeHandler
  stub_0 <- wrap_at $ \self _cmd -> do
    sp <- readHsData self
    rec_ <- deRefStablePtr sp :: IO GCLinearInputOverrides
  ...
  stub_7 <- wrap_at $ \self _cmd -> do
    sp <- readHsData self
    rec_ <- deRefStablePtr sp :: IO GCLinearInputOverrides
    case _physicalExtents rec_ of
      Nothing -> pure nullPtr
      Just f -> do
        r <- f 
        pure (castPtr (unRawId r) :: Ptr ObjCObject)
  addObjCMethod cls "physicalExtents" "@@:" stub_7
  ...
  stub_8 <- wrap_at $ \self _cmd -> do
    ...
    case _sources rec_ of
      ...
      Just f -> do
        r <- f 
        pure (castPtr (unRawId r) :: Ptr ObjCObject)
  addObjCMethod cls "sources" "@@:" stub_8
  ...
  rtsStub <- wrap_respondsToSel $ \self _cmd queriedSel -> do
    ...
    else if queriedSel == sel_physicalExtents then pure (maybe 0 (const 1) (_physicalExtents rec_))
...
newGCLinearInput :: GCLinearInputOverrides -> IO RawId
newGCLinearInput overrides = do
  inst <- class_createInstance gcLinearInputDelegateClass 0
