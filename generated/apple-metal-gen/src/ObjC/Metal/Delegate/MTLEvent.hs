...
-- | Generated delegate overrides for @\@protocol MTLEvent@.
--
-- Usage:
--
-- @
-- delegate <- newMTLEvent defaultMTLEventOverrides
--   { ... = Just $ \\arg0 -> ...
--   }
-- @
module ObjC.Metal.Delegate.MTLEvent
  ( MTLEventOverrides(..)
  , defaultMTLEventOverrides
  , newMTLEvent
  ) where
...
-- | Overrides record for @\@protocol MTLEvent@.
--
-- Each field corresponds to a protocol method.  'Nothing' means the
-- method is not implemented (the object will not respond to that
-- selector).  'Just' provides the Haskell implementation.
data MTLEventOverrides = MTLEventOverrides
...
defaultMTLEventOverrides :: MTLEventOverrides
defaultMTLEventOverrides = MTLEventOverrides
  { _device = Nothing
  , _label = Nothing
  , _setLabel = Nothing
...
{-# NOINLINE mtlEventDelegateClass #-}
mtlEventDelegateClass :: Class
mtlEventDelegateClass = unsafePerformIO $ do
  superCls <- getRequiredClass "NSObject"
  cls <- withCString "HsMTLEvent" $ \n ->
  ...
  stub_0 <- wrap_at $ \self _cmd -> do
    sp <- readHsData self
    rec_ <- deRefStablePtr sp :: IO MTLEventOverrides
...
newMTLEvent :: MTLEventOverrides -> IO RawId
newMTLEvent overrides = do
  inst <- class_createInstance mtlEventDelegateClass 0
