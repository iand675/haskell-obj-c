...
initWithDuration_animationCurve nsAnimation  duration animationCurve =
  sendMsg nsAnimation (mkSelector "initWithDuration:animationCurve:") (retPtr retVoid) [argCDouble (fromIntegral duration), argCULong (coerce animationCurve)] >>= ownedObject . castPtr
...
addProgressMark nsAnimation  progressMark =
  sendMsg nsAnimation (mkSelector "addProgressMark:") retVoid [argCFloat (fromIntegral progressMark)]

-- | @- removeProgressMark:@
removeProgressMark :: IsNSAnimation nsAnimation => nsAnimation -> CFloat -> IO ()
removeProgressMark nsAnimation  progressMark =
  sendMsg nsAnimation (mkSelector "removeProgressMark:") retVoid [argCFloat (fromIntegral progressMark)]
...
withObjCPtr animation $ \raw_animation ->
    sendMsg nsAnimation (mkSelector "startWhenAnimation:reachesProgress:") retVoid [argPtr (castPtr raw_animation :: Ptr ()), argCFloat (fromIntegral startProgress)]
...
withObjCPtr animation $ \raw_animation ->
    sendMsg nsAnimation (mkSelector "stopWhenAnimation:reachesProgress:") retVoid [argPtr (castPtr raw_animation :: Ptr ()), argCFloat (fromIntegral stopProgress)]
...
setCurrentProgress nsAnimation  value =
  sendMsg nsAnimation (mkSelector "setCurrentProgress:") retVoid [argCFloat (fromIntegral value)]
...
setDuration nsAnimation  value =
  sendMsg nsAnimation (mkSelector "setDuration:") retVoid [argCDouble (fromIntegral value)]
...
setFrameRate nsAnimation  value =
  sendMsg nsAnimation (mkSelector "setFrameRate:") retVoid [argCFloat (fromIntegral value)]
