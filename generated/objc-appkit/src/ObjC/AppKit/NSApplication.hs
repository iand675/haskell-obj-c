...
windowWithWindowNumber nsApplication  windowNum =
  sendMsg nsApplication (mkSelector "windowWithWindowNumber:") (retPtr retVoid) [argCLong (fromIntegral windowNum)] >>= retainedObject . castPtr
...
stopModalWithCode nsApplication  returnCode =
  sendMsg nsApplication (mkSelector "stopModalWithCode:") retVoid [argCLong (fromIntegral returnCode)]
...
cancelUserAttentionRequest nsApplication  request =
  sendMsg nsApplication (mkSelector "cancelUserAttentionRequest:") retVoid [argCLong (fromIntegral request)]
...
withObjCPtr sheet $ \raw_sheet ->
    sendMsg nsApplication (mkSelector "endSheet:returnCode:") retVoid [argPtr (castPtr raw_sheet :: Ptr ()), argCLong (fromIntegral returnCode)]
