...
setSelectionIndex nsArrayController  index =
  fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsArrayController (mkSelector "setSelectionIndex:") retCULong [argCULong (fromIntegral index)]
...
insertObject_atArrangedObjectIndex nsArrayController  object index =
  sendMsg nsArrayController (mkSelector "insertObject:atArrangedObjectIndex:") retVoid [argPtr (castPtr (unRawId object) :: Ptr ()), argCULong (fromIntegral index)]
...
removeObjectAtArrangedObjectIndex nsArrayController  index =
  sendMsg nsArrayController (mkSelector "removeObjectAtArrangedObjectIndex:") retVoid [argCULong (fromIntegral index)]
