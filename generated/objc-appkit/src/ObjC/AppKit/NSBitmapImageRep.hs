...
withObjCPtr colorSpaceName $ \raw_colorSpaceName ->
    sendMsg nsBitmapImageRep (mkSelector "initWithBitmapDataPlanes:pixelsWide:pixelsHigh:bitsPerSample:samplesPerPixel:hasAlpha:isPlanar:colorSpaceName:bytesPerRow:bitsPerPixel:") (retPtr retVoid) [argPtr planes, argCLong (fromIntegral width), argCLong (fromIntegral height), argCLong (fromIntegral bps), argCLong (fromIntegral spp), argCULong (if alpha then 1 else 0), argCULong (if isPlanar then 1 else 0), argPtr (castPtr raw_colorSpaceName :: Ptr ()), argCLong (fromIntegral rBytes), argCLong (fromIntegral pBits)] >>= ownedObject . castPtr
...
withObjCPtr colorSpaceName $ \raw_colorSpaceName ->
    sendMsg nsBitmapImageRep (mkSelector "initWithBitmapDataPlanes:pixelsWide:pixelsHigh:bitsPerSample:samplesPerPixel:hasAlpha:isPlanar:colorSpaceName:bitmapFormat:bytesPerRow:bitsPerPixel:") (retPtr retVoid) [argPtr planes, argCLong (fromIntegral width), argCLong (fromIntegral height), argCLong (fromIntegral bps), argCLong (fromIntegral spp), argCULong (if alpha then 1 else 0), argCULong (if isPlanar then 1 else 0), argPtr (castPtr raw_colorSpaceName :: Ptr ()), argCULong (coerce bitmapFormat), argCLong (fromIntegral rBytes), argCLong (fromIntegral pBits)] >>= ownedObject . castPtr
...
setCompression_factor nsBitmapImageRep  compression factor =
  sendMsg nsBitmapImageRep (mkSelector "setCompression:factor:") retVoid [argCULong (coerce compression), argCFloat (fromIntegral factor)]
...
tiffRepresentationUsingCompression_factor nsBitmapImageRep  comp factor =
  sendMsg nsBitmapImageRep (mkSelector "TIFFRepresentationUsingCompression:factor:") (retPtr retVoid) [argCULong (coerce comp), argCFloat (fromIntegral factor)] >>= retainedObject . castPtr
...
tiffRepresentationOfImageRepsInArray_usingCompression_factor array comp factor =
  do
    cls' <- getRequiredClass "NSBitmapImageRep"
    withObjCPtr array $ \raw_array ->
      sendClassMsg cls' (mkSelector "TIFFRepresentationOfImageRepsInArray:usingCompression:factor:") (retPtr retVoid) [argPtr (castPtr raw_array :: Ptr ()), argCULong (coerce comp), argCFloat (fromIntegral factor)] >>= retainedObject . castPtr
...
withObjCPtr midPointColor $ \raw_midPointColor ->
  withObjCPtr shadowColor $ \raw_shadowColor ->
    withObjCPtr lightColor $ \raw_lightColor ->
        sendMsg nsBitmapImageRep (mkSelector "colorizeByMappingGray:toColor:blackMapping:whiteMapping:") retVoid [argCDouble (fromIntegral midPoint), argPtr (castPtr raw_midPointColor :: Ptr ()), argPtr (castPtr raw_shadowColor :: Ptr ()), argPtr (castPtr raw_lightColor :: Ptr ())]
...
withObjCPtr color $ \raw_color ->
    sendMsg nsBitmapImageRep (mkSelector "setColor:atX:y:") retVoid [argPtr (castPtr raw_color :: Ptr ()), argCLong (fromIntegral x), argCLong (fromIntegral y)]

-- | @- colorAtX:y:@
colorAtX_y :: IsNSBitmapImageRep nsBitmapImageRep => nsBitmapImageRep -> CLong -> CLong -> IO (Id NSColor)
colorAtX_y nsBitmapImageRep  x y =
  sendMsg nsBitmapImageRep (mkSelector "colorAtX:y:") (retPtr retVoid) [argCLong (fromIntegral x), argCLong (fromIntegral y)] >>= retainedObject . castPtr
...
getPixel_atX_y nsBitmapImageRep  p x y =
  sendMsg nsBitmapImageRep (mkSelector "getPixel:atX:y:") retVoid [argPtr p, argCLong (fromIntegral x), argCLong (fromIntegral y)]
...
setPixel_atX_y nsBitmapImageRep  p x y =
  sendMsg nsBitmapImageRep (mkSelector "setPixel:atX:y:") retVoid [argPtr p, argCLong (fromIntegral x), argCLong (fromIntegral y)]
