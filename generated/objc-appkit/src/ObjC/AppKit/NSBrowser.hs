...
itemAtRow_inColumn nsBrowser  row column =
  fmap (RawId . castPtr) $ sendMsg nsBrowser (mkSelector "itemAtRow:inColumn:") (retPtr retVoid) [argCLong (fromIntegral row), argCLong (fromIntegral column)]

-- | @- indexPathForColumn:@
indexPathForColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO (Id NSIndexPath)
indexPathForColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "indexPathForColumn:") (retPtr retVoid) [argCLong (fromIntegral column)] >>= retainedObject . castPtr
...
withObjCPtr rowIndexes $ \raw_rowIndexes ->
    sendMsg nsBrowser (mkSelector "reloadDataForRowIndexes:inColumn:") retVoid [argPtr (castPtr raw_rowIndexes :: Ptr ()), argCLong (fromIntegral column)]

-- | @- parentForItemsInColumn:@
parentForItemsInColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO RawId
parentForItemsInColumn nsBrowser  column =
  fmap (RawId . castPtr) $ sendMsg nsBrowser (mkSelector "parentForItemsInColumn:") (retPtr retVoid) [argCLong (fromIntegral column)]
...
scrollRowToVisible_inColumn nsBrowser  row column =
  sendMsg nsBrowser (mkSelector "scrollRowToVisible:inColumn:") retVoid [argCLong (fromIntegral row), argCLong (fromIntegral column)]
...
withObjCPtr string $ \raw_string ->
    sendMsg nsBrowser (mkSelector "setTitle:ofColumn:") retVoid [argPtr (castPtr raw_string :: Ptr ()), argCLong (fromIntegral column)]

-- | @- titleOfColumn:@
titleOfColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO (Id NSString)
titleOfColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "titleOfColumn:") (retPtr retVoid) [argCLong (fromIntegral column)] >>= retainedObject . castPtr
...
pathToColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "pathToColumn:") (retPtr retVoid) [argCLong (fromIntegral column)] >>= retainedObject . castPtr

-- | @- selectedCellInColumn:@
selectedCellInColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO RawId
selectedCellInColumn nsBrowser  column =
  fmap (RawId . castPtr) $ sendMsg nsBrowser (mkSelector "selectedCellInColumn:") (retPtr retVoid) [argCLong (fromIntegral column)]

-- | @- selectRow:inColumn:@
selectRow_inColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> CLong -> IO ()
selectRow_inColumn nsBrowser  row column =
  sendMsg nsBrowser (mkSelector "selectRow:inColumn:") retVoid [argCLong (fromIntegral row), argCLong (fromIntegral column)]

-- | @- selectedRowInColumn:@
selectedRowInColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO CLong
selectedRowInColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "selectedRowInColumn:") retCLong [argCLong (fromIntegral column)]
...
withObjCPtr indexes $ \raw_indexes ->
    sendMsg nsBrowser (mkSelector "selectRowIndexes:inColumn:") retVoid [argPtr (castPtr raw_indexes :: Ptr ()), argCLong (fromIntegral column)]
...
selectedRowIndexesInColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "selectedRowIndexesInColumn:") (retPtr retVoid) [argCLong (fromIntegral column)] >>= retainedObject . castPtr

-- | @- reloadColumn:@
reloadColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO ()
reloadColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "reloadColumn:") retVoid [argCLong (fromIntegral column)]
...
scrollColumnsRightBy nsBrowser  shiftAmount =
  sendMsg nsBrowser (mkSelector "scrollColumnsRightBy:") retVoid [argCLong (fromIntegral shiftAmount)]

-- | @- scrollColumnsLeftBy:@
scrollColumnsLeftBy :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO ()
scrollColumnsLeftBy nsBrowser  shiftAmount =
  sendMsg nsBrowser (mkSelector "scrollColumnsLeftBy:") retVoid [argCLong (fromIntegral shiftAmount)]

-- | @- scrollColumnToVisible:@
scrollColumnToVisible :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO ()
scrollColumnToVisible nsBrowser  column =
  sendMsg nsBrowser (mkSelector "scrollColumnToVisible:") retVoid [argCLong (fromIntegral column)]
...
loadedCellAtRow_column nsBrowser  row col =
  fmap (RawId . castPtr) $ sendMsg nsBrowser (mkSelector "loadedCellAtRow:column:") (retPtr retVoid) [argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
titleFrameOfColumn nsBrowser  column =
  sendMsgStret nsBrowser (mkSelector "titleFrameOfColumn:") retNSRect [argCLong (fromIntegral column)]
...
drawTitleOfColumn_inRect nsBrowser  column rect =
  sendMsg nsBrowser (mkSelector "drawTitleOfColumn:inRect:") retVoid [argCLong (fromIntegral column), argNSRect rect]

-- | @- frameOfColumn:@
frameOfColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO NSRect
frameOfColumn nsBrowser  column =
  sendMsgStret nsBrowser (mkSelector "frameOfColumn:") retNSRect [argCLong (fromIntegral column)]

-- | @- frameOfInsideOfColumn:@
frameOfInsideOfColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO NSRect
frameOfInsideOfColumn nsBrowser  column =
  sendMsgStret nsBrowser (mkSelector "frameOfInsideOfColumn:") retNSRect [argCLong (fromIntegral column)]

-- | @- frameOfRow:inColumn:@
frameOfRow_inColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> CLong -> IO NSRect
frameOfRow_inColumn nsBrowser  row column =
  sendMsgStret nsBrowser (mkSelector "frameOfRow:inColumn:") retNSRect [argCLong (fromIntegral row), argCLong (fromIntegral column)]
...
columnWidthForColumnContentWidth nsBrowser  columnContentWidth =
  sendMsg nsBrowser (mkSelector "columnWidthForColumnContentWidth:") retCDouble [argCDouble (fromIntegral columnContentWidth)]
...
columnContentWidthForColumnWidth nsBrowser  columnWidth =
  sendMsg nsBrowser (mkSelector "columnContentWidthForColumnWidth:") retCDouble [argCDouble (fromIntegral columnWidth)]

-- | @- setWidth:ofColumn:@
setWidth_ofColumn :: IsNSBrowser nsBrowser => nsBrowser -> CDouble -> CLong -> IO ()
setWidth_ofColumn nsBrowser  columnWidth columnIndex =
  sendMsg nsBrowser (mkSelector "setWidth:ofColumn:") retVoid [argCDouble (fromIntegral columnWidth), argCLong (fromIntegral columnIndex)]

-- | @- widthOfColumn:@
widthOfColumn :: IsNSBrowser nsBrowser => nsBrowser -> CLong -> IO CDouble
widthOfColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "widthOfColumn:") retCDouble [argCLong (fromIntegral column)]
...
withObjCPtr indexSet $ \raw_indexSet ->
    sendMsg nsBrowser (mkSelector "noteHeightOfRowsWithIndexesChanged:inColumn:") retVoid [argPtr (castPtr raw_indexSet :: Ptr ()), argCLong (fromIntegral columnIndex)]

-- | @- setDefaultColumnWidth:@
setDefaultColumnWidth :: IsNSBrowser nsBrowser => nsBrowser -> CDouble -> IO ()
setDefaultColumnWidth nsBrowser  columnWidth =
  sendMsg nsBrowser (mkSelector "setDefaultColumnWidth:") retVoid [argCDouble (fromIntegral columnWidth)]
...
withObjCPtr rowIndexes $ \raw_rowIndexes ->
  withObjCPtr event $ \raw_event ->
      fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsBrowser (mkSelector "canDragRowsWithIndexes:inColumn:withEvent:") retCULong [argPtr (castPtr raw_rowIndexes :: Ptr ()), argCLong (fromIntegral column), argPtr (castPtr raw_event :: Ptr ())]
...
withObjCPtr rowIndexes $ \raw_rowIndexes ->
  withObjCPtr event $ \raw_event ->
      sendMsg nsBrowser (mkSelector "draggingImageForRowsWithIndexes:inColumn:withEvent:offset:") (retPtr retVoid) [argPtr (castPtr raw_rowIndexes :: Ptr ()), argCLong (fromIntegral column), argPtr (castPtr raw_event :: Ptr ()), argPtr dragImageOffset] >>= retainedObject . castPtr
...
displayColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "displayColumn:") retVoid [argCLong (fromIntegral column)]
...
matrixInColumn nsBrowser  column =
  sendMsg nsBrowser (mkSelector "matrixInColumn:") (retPtr retVoid) [argCLong (fromIntegral column)] >>= retainedObject . castPtr
...
setMinColumnWidth nsBrowser  value =
  sendMsg nsBrowser (mkSelector "setMinColumnWidth:") retVoid [argCDouble (fromIntegral value)]
...
setMaxVisibleColumns nsBrowser  value =
  sendMsg nsBrowser (mkSelector "setMaxVisibleColumns:") retVoid [argCLong (fromIntegral value)]
...
setLastColumn nsBrowser  value =
  sendMsg nsBrowser (mkSelector "setLastColumn:") retVoid [argCLong (fromIntegral value)]
...
setRowHeight nsBrowser  value =
  sendMsg nsBrowser (mkSelector "setRowHeight:") retVoid [argCDouble (fromIntegral value)]
