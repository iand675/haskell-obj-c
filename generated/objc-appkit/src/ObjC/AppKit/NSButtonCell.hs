...
setPeriodicDelay_interval nsButtonCell  delay interval =
  sendMsg nsButtonCell (mkSelector "setPeriodicDelay:interval:") retVoid [argCFloat (fromIntegral delay), argCFloat (fromIntegral interval)]
...
setAlternateMnemonicLocation nsButtonCell  location =
  sendMsg nsButtonCell (mkSelector "setAlternateMnemonicLocation:") retVoid [argCULong (fromIntegral location)]
...
withObjCPtr fontName $ \raw_fontName ->
    sendMsg nsButtonCell (mkSelector "setKeyEquivalentFont:size:") retVoid [argPtr (castPtr raw_fontName :: Ptr ()), argCDouble (fromIntegral fontSize)]
