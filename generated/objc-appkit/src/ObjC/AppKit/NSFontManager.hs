...
withObjCPtr family_ $ \raw_family_ ->
    sendMsg nsFontManager (mkSelector "fontWithFamily:traits:weight:size:") (retPtr retVoid) [argPtr (castPtr raw_family_ :: Ptr ()), argCULong (coerce traits), argCLong (fromIntegral weight), argCDouble (fromIntegral size)] >>= retainedObject . castPtr
...
withObjCPtr fontObj $ \raw_fontObj ->
    sendMsg nsFontManager (mkSelector "convertFont:toSize:") (retPtr retVoid) [argPtr (castPtr raw_fontObj :: Ptr ()), argCDouble (fromIntegral size)] >>= retainedObject . castPtr
