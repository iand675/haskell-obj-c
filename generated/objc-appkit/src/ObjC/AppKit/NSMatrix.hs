...
withObjCPtr cell $ \raw_cell ->
    sendMsg nsMatrix (mkSelector "initWithFrame:mode:prototype:numberOfRows:numberOfColumns:") (retPtr retVoid) [argNSRect frameRect, argCULong (coerce mode), argPtr (castPtr raw_cell :: Ptr ()), argCLong (fromIntegral rowsHigh), argCLong (fromIntegral colsWide)] >>= ownedObject . castPtr
...
initWithFrame_mode_cellClass_numberOfRows_numberOfColumns nsMatrix  frameRect mode factoryId rowsHigh colsWide =
  sendMsg nsMatrix (mkSelector "initWithFrame:mode:cellClass:numberOfRows:numberOfColumns:") (retPtr retVoid) [argNSRect frameRect, argCULong (coerce mode), argPtr (unClass factoryId), argCLong (fromIntegral rowsHigh), argCLong (fromIntegral colsWide)] >>= ownedObject . castPtr
...
makeCellAtRow_column nsMatrix  row col =
  sendMsg nsMatrix (mkSelector "makeCellAtRow:column:") (retPtr retVoid) [argCLong (fromIntegral row), argCLong (fromIntegral col)] >>= retainedObject . castPtr
...
setSelectionFrom_to_anchor_highlight nsMatrix  startPos endPos anchorPos lit =
  sendMsg nsMatrix (mkSelector "setSelectionFrom:to:anchor:highlight:") retVoid [argCLong (fromIntegral startPos), argCLong (fromIntegral endPos), argCLong (fromIntegral anchorPos), argCULong (if lit then 1 else 0)]
...
selectCellAtRow_column nsMatrix  row col =
  sendMsg nsMatrix (mkSelector "selectCellAtRow:column:") retVoid [argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
selectCellWithTag nsMatrix  tag =
  fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsMatrix (mkSelector "selectCellWithTag:") retCULong [argCLong (fromIntegral tag)]
...
setState_atRow_column nsMatrix  value row col =
  sendMsg nsMatrix (mkSelector "setState:atRow:column:") retVoid [argCLong (fromIntegral value), argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
cellAtRow_column nsMatrix  row col =
  sendMsg nsMatrix (mkSelector "cellAtRow:column:") (retPtr retVoid) [argCLong (fromIntegral row), argCLong (fromIntegral col)] >>= retainedObject . castPtr
...
cellFrameAtRow_column nsMatrix  row col =
  sendMsgStret nsMatrix (mkSelector "cellFrameAtRow:column:") retNSRect [argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
renewRows_columns nsMatrix  newRows newCols =
  sendMsg nsMatrix (mkSelector "renewRows:columns:") retVoid [argCLong (fromIntegral newRows), argCLong (fromIntegral newCols)]
...
withObjCPtr newCell $ \raw_newCell ->
    sendMsg nsMatrix (mkSelector "putCell:atRow:column:") retVoid [argPtr (castPtr raw_newCell :: Ptr ()), argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
insertRow nsMatrix  row =
  sendMsg nsMatrix (mkSelector "insertRow:") retVoid [argCLong (fromIntegral row)]
...
withObjCPtr newCells $ \raw_newCells ->
    sendMsg nsMatrix (mkSelector "insertRow:withCells:") retVoid [argCLong (fromIntegral row), argPtr (castPtr raw_newCells :: Ptr ())]

-- | @- removeRow:@
removeRow :: IsNSMatrix nsMatrix => nsMatrix -> CLong -> IO ()
removeRow nsMatrix  row =
  sendMsg nsMatrix (mkSelector "removeRow:") retVoid [argCLong (fromIntegral row)]
...
insertColumn nsMatrix  column =
  sendMsg nsMatrix (mkSelector "insertColumn:") retVoid [argCLong (fromIntegral column)]
...
withObjCPtr newCells $ \raw_newCells ->
    sendMsg nsMatrix (mkSelector "insertColumn:withCells:") retVoid [argCLong (fromIntegral column), argPtr (castPtr raw_newCells :: Ptr ())]

-- | @- removeColumn:@
removeColumn :: IsNSMatrix nsMatrix => nsMatrix -> CLong -> IO ()
removeColumn nsMatrix  col =
  sendMsg nsMatrix (mkSelector "removeColumn:") retVoid [argCLong (fromIntegral col)]

-- | @- cellWithTag:@
cellWithTag :: IsNSMatrix nsMatrix => nsMatrix -> CLong -> IO (Id NSCell)
cellWithTag nsMatrix  tag =
  sendMsg nsMatrix (mkSelector "cellWithTag:") (retPtr retVoid) [argCLong (fromIntegral tag)] >>= retainedObject . castPtr
...
drawCellAtRow_column nsMatrix  row col =
  sendMsg nsMatrix (mkSelector "drawCellAtRow:column:") retVoid [argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
highlightCell_atRow_column nsMatrix  flag row col =
  sendMsg nsMatrix (mkSelector "highlightCell:atRow:column:") retVoid [argCULong (if flag then 1 else 0), argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
scrollCellToVisibleAtRow_column nsMatrix  row col =
  sendMsg nsMatrix (mkSelector "scrollCellToVisibleAtRow:column:") retVoid [argCLong (fromIntegral row), argCLong (fromIntegral col)]
...
selectTextAtRow_column nsMatrix  row col =
  sendMsg nsMatrix (mkSelector "selectTextAtRow:column:") (retPtr retVoid) [argCLong (fromIntegral row), argCLong (fromIntegral col)] >>= retainedObject . castPtr
