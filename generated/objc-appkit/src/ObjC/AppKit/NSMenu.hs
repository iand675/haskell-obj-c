...
withObjCPtr newItem $ \raw_newItem ->
    sendMsg nsMenu (mkSelector "insertItem:atIndex:") retVoid [argPtr (castPtr raw_newItem :: Ptr ()), argCLong (fromIntegral index)]
...
withObjCPtr string $ \raw_string ->
  withObjCPtr charCode $ \raw_charCode ->
      sendMsg nsMenu (mkSelector "insertItemWithTitle:action:keyEquivalent:atIndex:") (retPtr retVoid) [argPtr (castPtr raw_string :: Ptr ()), argPtr (unSelector selector), argPtr (castPtr raw_charCode :: Ptr ()), argCLong (fromIntegral index)] >>= retainedObject . castPtr
...
removeItemAtIndex nsMenu  index =
  sendMsg nsMenu (mkSelector "removeItemAtIndex:") retVoid [argCLong (fromIntegral index)]
...
itemAtIndex nsMenu  index =
  sendMsg nsMenu (mkSelector "itemAtIndex:") (retPtr retVoid) [argCLong (fromIntegral index)] >>= retainedObject . castPtr
...
indexOfItemWithTag nsMenu  tag =
  sendMsg nsMenu (mkSelector "indexOfItemWithTag:") retCLong [argCLong (fromIntegral tag)]
...
itemWithTag nsMenu  tag =
  sendMsg nsMenu (mkSelector "itemWithTag:") (retPtr retVoid) [argCLong (fromIntegral tag)] >>= retainedObject . castPtr
...
performActionForItemAtIndex nsMenu  index =
  sendMsg nsMenu (mkSelector "performActionForItemAtIndex:") retVoid [argCLong (fromIntegral index)]
...
setMinimumWidth nsMenu  value =
  sendMsg nsMenu (mkSelector "setMinimumWidth:") retVoid [argCDouble (fromIntegral value)]
