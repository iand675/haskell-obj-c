...
withObjCPtr view $ \raw_view ->
    sendMsg nsObject (mkSelector "view:stringForToolTip:point:userData:") (retPtr retVoid) [argPtr (castPtr raw_view :: Ptr ()), argCLong (fromIntegral tag), argNSPoint point, argPtr data_] >>= retainedObject . castPtr

-- | @- layer:shouldInheritContentsScale:fromWindow:@
layer_shouldInheritContentsScale_fromWindow :: (IsNSObject nsObject, IsCALayer layer, IsNSWindow window) => nsObject -> layer -> CDouble -> window -> IO Bool
...
withObjCPtr layer $ \raw_layer ->
  withObjCPtr window $ \raw_window ->
      fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsObject (mkSelector "layer:shouldInheritContentsScale:fromWindow:") retCULong [argPtr (castPtr raw_layer :: Ptr ()), argCDouble (fromIntegral newScale), argPtr (castPtr raw_window :: Ptr ())]
...
withObjCPtr attribute $ \raw_attribute ->
    sendMsg nsObject (mkSelector "accessibilityArrayAttributeValues:index:maxCount:") (retPtr retVoid) [argPtr (castPtr raw_attribute :: Ptr ()), argCULong (fromIntegral index), argCULong (fromIntegral maxCount)] >>= retainedObject . castPtr
