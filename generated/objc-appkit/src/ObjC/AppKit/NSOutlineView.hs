...
child_ofItem nsOutlineView  index item =
  fmap (RawId . castPtr) $ sendMsg nsOutlineView (mkSelector "child:ofItem:") (retPtr retVoid) [argCLong (fromIntegral index), argPtr (castPtr (unRawId item) :: Ptr ())]
...
itemAtRow nsOutlineView  row =
  fmap (RawId . castPtr) $ sendMsg nsOutlineView (mkSelector "itemAtRow:") (retPtr retVoid) [argCLong (fromIntegral row)]
...
levelForRow nsOutlineView  row =
  sendMsg nsOutlineView (mkSelector "levelForRow:") retCLong [argCLong (fromIntegral row)]
...
frameOfOutlineCellAtRow nsOutlineView  row =
  sendMsgStret nsOutlineView (mkSelector "frameOfOutlineCellAtRow:") retNSRect [argCLong (fromIntegral row)]
...
setDropItem_dropChildIndex nsOutlineView  item index =
  sendMsg nsOutlineView (mkSelector "setDropItem:dropChildIndex:") retVoid [argPtr (castPtr (unRawId item) :: Ptr ()), argCLong (fromIntegral index)]
...
moveItemAtIndex_inParent_toIndex_inParent nsOutlineView  fromIndex oldParent toIndex newParent =
  sendMsg nsOutlineView (mkSelector "moveItemAtIndex:inParent:toIndex:inParent:") retVoid [argCLong (fromIntegral fromIndex), argPtr (castPtr (unRawId oldParent) :: Ptr ()), argCLong (fromIntegral toIndex), argPtr (castPtr (unRawId newParent) :: Ptr ())]
...
moveRowAtIndex_toIndex nsOutlineView  oldIndex newIndex =
  sendMsg nsOutlineView (mkSelector "moveRowAtIndex:toIndex:") retVoid [argCLong (fromIntegral oldIndex), argCLong (fromIntegral newIndex)]
...
setIndentationPerLevel nsOutlineView  value =
  sendMsg nsOutlineView (mkSelector "setIndentationPerLevel:") retVoid [argCDouble (fromIntegral value)]
