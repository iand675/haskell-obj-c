...
locationFromLocation_withOffset nsTextContentStorage  location offset =
  fmap (RawId . castPtr) $ sendMsg nsTextContentStorage (mkSelector "locationFromLocation:withOffset:") (retPtr retVoid) [argPtr (castPtr (unRawId location) :: Ptr ()), argCLong (fromIntegral offset)]
