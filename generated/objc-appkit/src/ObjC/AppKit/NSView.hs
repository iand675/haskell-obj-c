...
rotateByAngle nsView  angle =
  sendMsg nsView (mkSelector "rotateByAngle:") retVoid [argCDouble (fromIntegral angle)]
...
viewWithTag nsView  tag =
  sendMsg nsView (mkSelector "viewWithTag:") (retPtr retVoid) [argCLong (fromIntegral tag)] >>= retainedObject . castPtr
...
removeToolTip nsView  tag =
  sendMsg nsView (mkSelector "removeToolTip:") retVoid [argCLong (fromIntegral tag)]
...
withObjCPtr ruler $ \raw_ruler ->
  withObjCPtr marker $ \raw_marker ->
      sendMsg nsView (mkSelector "rulerView:willMoveMarker:toLocation:") retCDouble [argPtr (castPtr raw_ruler :: Ptr ()), argPtr (castPtr raw_marker :: Ptr ()), argCDouble (fromIntegral location)]
...
withObjCPtr ruler $ \raw_ruler ->
  withObjCPtr marker $ \raw_marker ->
      sendMsg nsView (mkSelector "rulerView:willAddMarker:atLocation:") retCDouble [argPtr (castPtr raw_ruler :: Ptr ()), argPtr (castPtr raw_marker :: Ptr ()), argCDouble (fromIntegral location)]
...
withObjCPtr ruler $ \raw_ruler ->
    sendMsgStret nsView (mkSelector "rulerView:pointForLocation:") retNSPoint [argPtr (castPtr raw_ruler :: Ptr ()), argCDouble (fromIntegral point)]
...
setContentHuggingPriority_forOrientation nsView  priority orientation =
  sendMsg nsView (mkSelector "setContentHuggingPriority:forOrientation:") retVoid [argCFloat (fromIntegral priority), argCLong (coerce orientation)]
...
setContentCompressionResistancePriority_forOrientation nsView  priority orientation =
  sendMsg nsView (mkSelector "setContentCompressionResistancePriority:forOrientation:") retVoid [argCFloat (fromIntegral priority), argCLong (coerce orientation)]
...
removeTrackingRect nsView  tag =
  sendMsg nsView (mkSelector "removeTrackingRect:") retVoid [argCLong (fromIntegral tag)]
...
adjustPageWidthNew_left_right_limit nsView  newRight oldLeft oldRight rightLimit =
  sendMsg nsView (mkSelector "adjustPageWidthNew:left:right:limit:") retVoid [argPtr newRight, argCDouble (fromIntegral oldLeft), argCDouble (fromIntegral oldRight), argCDouble (fromIntegral rightLimit)]
...
adjustPageHeightNew_top_bottom_limit nsView  newBottom oldTop oldBottom bottomLimit =
  sendMsg nsView (mkSelector "adjustPageHeightNew:top:bottom:limit:") retVoid [argPtr newBottom, argCDouble (fromIntegral oldTop), argCDouble (fromIntegral oldBottom), argCDouble (fromIntegral bottomLimit)]

-- | @- rectForPage:@
rectForPage :: IsNSView nsView => nsView -> CLong -> IO NSRect
rectForPage nsView  page =
  sendMsgStret nsView (mkSelector "rectForPage:") retNSRect [argCLong (fromIntegral page)]
...
setFrameRotation nsView  value =
  sendMsg nsView (mkSelector "setFrameRotation:") retVoid [argCDouble (fromIntegral value)]
...
setFrameCenterRotation nsView  value =
  sendMsg nsView (mkSelector "setFrameCenterRotation:") retVoid [argCDouble (fromIntegral value)]
...
setBoundsRotation nsView  value =
  sendMsg nsView (mkSelector "setBoundsRotation:") retVoid [argCDouble (fromIntegral value)]
...
setAlphaValue nsView  value =
  sendMsg nsView (mkSelector "setAlphaValue:") retVoid [argCDouble (fromIntegral value)]
