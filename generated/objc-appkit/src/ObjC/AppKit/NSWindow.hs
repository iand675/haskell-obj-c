...
withObjCPtr childViewController $ \raw_childViewController ->
    sendMsg nsWindow (mkSelector "insertTitlebarAccessoryViewController:atIndex:") retVoid [argPtr (castPtr raw_childViewController :: Ptr ()), argCLong (fromIntegral index)]
...
removeTitlebarAccessoryViewControllerAtIndex nsWindow  index =
  sendMsg nsWindow (mkSelector "removeTitlebarAccessoryViewControllerAtIndex:") retVoid [argCLong (fromIntegral index)]
...
setContentBorderThickness_forEdge nsWindow  thickness edge =
  sendMsg nsWindow (mkSelector "setContentBorderThickness:forEdge:") retVoid [argCDouble (fromIntegral thickness), argCULong (coerce edge)]
...
orderWindow_relativeTo nsWindow  place otherWin =
  sendMsg nsWindow (mkSelector "orderWindow:relativeTo:") retVoid [argCLong (coerce place), argCLong (fromIntegral otherWin)]
...
withObjCPtr sheetWindow $ \raw_sheetWindow ->
    sendMsg nsWindow (mkSelector "endSheet:returnCode:") retVoid [argPtr (castPtr raw_sheetWindow :: Ptr ()), argCLong (fromIntegral returnCode)]
...
windowNumberAtPoint_belowWindowWithWindowNumber point windowNumber =
  do
    cls' <- getRequiredClass "NSWindow"
    sendClassMsg cls' (mkSelector "windowNumberAtPoint:belowWindowWithWindowNumber:") retCLong [argNSPoint point, argCLong (fromIntegral windowNumber)]
...
withObjCPtr mode $ \raw_mode ->
    sendMsg nsWindow (mkSelector "trackEventsMatchingMask:timeout:mode:handler:") retVoid [argCULong (coerce mask), argCDouble (fromIntegral timeout), argPtr (castPtr raw_mode :: Ptr ()), argPtr (castPtr trackingHandler :: Ptr ())]
...
setLevel nsWindow  value =
  sendMsg nsWindow (mkSelector "setLevel:") retVoid [argCLong (fromIntegral value)]
...
setAlphaValue nsWindow  value =
  sendMsg nsWindow (mkSelector "setAlphaValue:") retVoid [argCDouble (fromIntegral value)]
...
setOrderedIndex nsWindow  value =
  sendMsg nsWindow (mkSelector "setOrderedIndex:") retVoid [argCLong (fromIntegral value)]
