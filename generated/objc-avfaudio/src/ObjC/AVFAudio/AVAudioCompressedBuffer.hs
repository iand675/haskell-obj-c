...
withObjCPtr format $ \raw_format ->
    sendMsg avAudioCompressedBuffer (mkSelector "initWithFormat:packetCapacity:maximumPacketSize:") (retPtr retVoid) [argPtr (castPtr raw_format :: Ptr ()), argCUInt (fromIntegral packetCapacity), argCLong (fromIntegral maximumPacketSize)] >>= ownedObject . castPtr
...
withObjCPtr format $ \raw_format ->
    sendMsg avAudioCompressedBuffer (mkSelector "initWithFormat:packetCapacity:") (retPtr retVoid) [argPtr (castPtr raw_format :: Ptr ()), argCUInt (fromIntegral packetCapacity)] >>= ownedObject . castPtr
...
setPacketCount avAudioCompressedBuffer  value =
  sendMsg avAudioCompressedBuffer (mkSelector "setPacketCount:") retVoid [argCUInt (fromIntegral value)]
...
setByteLength avAudioCompressedBuffer  value =
  sendMsg avAudioCompressedBuffer (mkSelector "setByteLength:") retVoid [argCUInt (fromIntegral value)]
