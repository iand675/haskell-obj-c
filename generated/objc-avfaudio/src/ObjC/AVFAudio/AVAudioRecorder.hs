...
recordAtTime avAudioRecorder  time =
  fmap ((/= 0) :: CULong -> Bool) $ sendMsg avAudioRecorder (mkSelector "recordAtTime:") retCULong [argCDouble (fromIntegral time)]
...
recordForDuration avAudioRecorder  duration =
  fmap ((/= 0) :: CULong -> Bool) $ sendMsg avAudioRecorder (mkSelector "recordForDuration:") retCULong [argCDouble (fromIntegral duration)]
...
recordAtTime_forDuration avAudioRecorder  time duration =
  fmap ((/= 0) :: CULong -> Bool) $ sendMsg avAudioRecorder (mkSelector "recordAtTime:forDuration:") retCULong [argCDouble (fromIntegral time), argCDouble (fromIntegral duration)]
...
peakPowerForChannel avAudioRecorder  channelNumber =
  sendMsg avAudioRecorder (mkSelector "peakPowerForChannel:") retCFloat [argCULong (fromIntegral channelNumber)]
...
averagePowerForChannel avAudioRecorder  channelNumber =
  sendMsg avAudioRecorder (mkSelector "averagePowerForChannel:") retCFloat [argCULong (fromIntegral channelNumber)]
