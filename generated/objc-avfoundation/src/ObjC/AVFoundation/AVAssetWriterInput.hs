...
--
-- Generated bindings for @AVAssetWriterInput@.
module ObjC.AVFoundation.AVAssetWriterInput
  ( AVAssetWriterInput
  , IsAVAssetWriterInput(..)
  , init_
  , new
  , assetWriterInputWithMediaType_outputSettings
  , assetWriterInputWithMediaType_outputSettings_sourceFormatHint
  , initWithMediaType_outputSettings
  , initWithMediaType_outputSettings_sourceFormatHint
  , requestMediaDataWhenReadyOnQueue_usingBlock
  , appendSampleBuffer
  , markAsFinished
  , respondToEachPassDescriptionOnQueue_usingBlock
  , markCurrentPassAsFinished
  , canAddTrackAssociationWithTrackOfInput_type
  , addTrackAssociationWithTrackOfInput_type
  , mediaType
  , outputSettings
  , sourceFormatHint
  , metadata
...
--
-- - Parameter mediaType: The media type of samples that will be accepted by the input. Media types are defined in AVMediaFormat.h. - Parameter outputSettings: The settings used for encoding the media appended to the output. See AVAudioSettings.h for AVMediaTypeAudio or AVVideoSettings.h for AVMediaTypeVideo and for more information on how to construct an output settings dictionary. If you only require simple preset-based output settings, see AVOutputSettingsAssistant.
--
-- - Returns: An instance of AVAssetWriterInput.
--
-- ObjC selector: @+ assetWriterInputWithMediaType:outputSettings:@
assetWriterInputWithMediaType_outputSettings :: (IsNSString mediaType, IsNSDictionary outputSettings) => mediaType -> outputSettings -> IO (Id AVAssetWriterInput)
...
--
-- This method throws an exception for any of the following reasons: - the media type of the format description does not match the media type passed into this method - the width and height of video format hint are not positive - the output settings do not match the supplied media type - for video inputs, the output settings do not contain a required key (AVVideoCodecKey, AVVideoWidthKey, AVVideoHeightKey) - the output scaling mode is AVVideoScalingModeFit - the output settings contain AVSampleRateConverterAudioQualityKey or AVVideoDecompressionPropertiesKey
--
...
-- - Returns: An instance of AVAssetWriterInput.
--
-- ObjC selector: @+ assetWriterInputWithMediaType:outputSettings:sourceFormatHint:@
assetWriterInputWithMediaType_outputSettings_sourceFormatHint :: (IsNSString mediaType, IsNSDictionary outputSettings) => mediaType -> outputSettings -> RawId -> IO (Id AVAssetWriterInput)
...
--
-- - Parameter queue: The queue on which the block should be invoked. - Parameter block: The block the input should invoke to obtain media data.
--
...
--
-- 1. Query the value of the receiver's currentPassDescription property and reconfigure the source of media data (e.g. AVAssetReader) accordingly 2. Call -requestMediaDataWhenReadyOnQueue:usingBlock: to begin appending data for the current pass 3. Exit
--
...
--
-- - Parameter queue: The queue on which the block should be invoked. - Parameter block: A block the receiver should invoke whenever a new pass has begun.
--
...
--
-- - Parameter input: The instance of AVAssetWriterInput with a corresponding track to associate with track corresponding with the receiver. - Parameter trackAssociationType: The type of track association to test. Common track association types, such as AVTrackAssociationTypeTimecode, are defined in AVAssetTrack.h.
--
...
--
-- - Parameter input: The instance of AVAssetWriterInput with a corresponding track to associate with track corresponding to the receiver. - Parameter trackAssociationType: The type of track association to add. Common track association types, such as AVTrackAssociationTypeTimecode, are defined in AVAssetTrack.h.
--
...
setMediaTimeScale avAssetWriterInput  value =
  sendMsg avAssetWriterInput (mkSelector "setMediaTimeScale:") retVoid [argCInt (fromIntegral value)]
...
setPreferredMediaChunkAlignment avAssetWriterInput  value =
  sendMsg avAssetWriterInput (mkSelector "setPreferredMediaChunkAlignment:") retVoid [argCLong (fromIntegral value)]
...
setPreferredVolume avAssetWriterInput  value =
  sendMsg avAssetWriterInput (mkSelector "setPreferredVolume:") retVoid [argCFloat (fromIntegral value)]
