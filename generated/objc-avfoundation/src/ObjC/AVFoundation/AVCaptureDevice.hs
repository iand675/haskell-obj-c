...
setTransportControlsPlaybackMode_speed avCaptureDevice  mode speed =
  sendMsg avCaptureDevice (mkSelector "setTransportControlsPlaybackMode:speed:") retVoid [argCLong (coerce mode), argCFloat (fromIntegral speed)]
...
rampToVideoZoomFactor_withRate avCaptureDevice  factor rate =
  sendMsg avCaptureDevice (mkSelector "rampToVideoZoomFactor:withRate:") retVoid [argCDouble (fromIntegral factor), argCFloat (fromIntegral rate)]
...
setExposureTargetBias_completionHandler avCaptureDevice  bias handler =
  sendMsg avCaptureDevice (mkSelector "setExposureTargetBias:completionHandler:") retVoid [argCFloat (fromIntegral bias), argPtr (castPtr handler :: Ptr ())]
...
setFocusModeLockedWithLensPosition_completionHandler avCaptureDevice  lensPosition handler =
  sendMsg avCaptureDevice (mkSelector "setFocusModeLockedWithLensPosition:completionHandler:") retVoid [argCFloat (fromIntegral lensPosition), argPtr (castPtr handler :: Ptr ())]
...
setCinematicVideoTrackingFocusWithDetectedObjectID_focusMode avCaptureDevice  detectedObjectID focusMode =
  sendMsg avCaptureDevice (mkSelector "setCinematicVideoTrackingFocusWithDetectedObjectID:focusMode:") retVoid [argCLong (fromIntegral detectedObjectID), argCLong (coerce focusMode)]
...
withObjCPtr outError $ \raw_outError ->
    fmap ((/= 0) :: CULong -> Bool) $ sendMsg avCaptureDevice (mkSelector "setTorchModeOnWithLevel:error:") retCULong [argCFloat (fromIntegral torchLevel), argPtr (castPtr raw_outError :: Ptr ())]
...
setVideoZoomFactor avCaptureDevice  value =
  sendMsg avCaptureDevice (mkSelector "setVideoZoomFactor:") retVoid [argCDouble (fromIntegral value)]
