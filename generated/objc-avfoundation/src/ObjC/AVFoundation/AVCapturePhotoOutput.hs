...
isBayerRAWPixelFormat pixelFormat =
  do
    cls' <- getRequiredClass "AVCapturePhotoOutput"
    fmap ((/= 0) :: CULong -> Bool) $ sendClassMsg cls' (mkSelector "isBayerRAWPixelFormat:") retCULong [argCUInt (fromIntegral pixelFormat)]
...
isAppleProRAWPixelFormat pixelFormat =
  do
    cls' <- getRequiredClass "AVCapturePhotoOutput"
    fmap ((/= 0) :: CULong -> Bool) $ sendClassMsg cls' (mkSelector "isAppleProRAWPixelFormat:") retCULong [argCUInt (fromIntegral pixelFormat)]
...
withObjCPtr fileType $ \raw_fileType ->
    sendMsg avCapturePhotoOutput (mkSelector "supportedRawPhotoCodecTypesForRawPhotoPixelFormatType:fileType:") (retPtr retVoid) [argCUInt (fromIntegral pixelFormatType), argPtr (castPtr raw_fileType :: Ptr ())] >>= retainedObject . castPtr
