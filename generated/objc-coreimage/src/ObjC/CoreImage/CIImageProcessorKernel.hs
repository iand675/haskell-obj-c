...
formatForInputAtIndex inputIndex =
  do
    cls' <- getRequiredClass "CIImageProcessorKernel"
    sendClassMsg cls' (mkSelector "formatForInputAtIndex:") retCInt [argCInt (fromIntegral inputIndex)]
...
outputFormatAtIndex_arguments outputIndex arguments =
  do
    cls' <- getRequiredClass "CIImageProcessorKernel"
    withObjCPtr arguments $ \raw_arguments ->
      sendClassMsg cls' (mkSelector "outputFormatAtIndex:arguments:") retCInt [argCInt (fromIntegral outputIndex), argPtr (castPtr raw_arguments :: Ptr ())]
