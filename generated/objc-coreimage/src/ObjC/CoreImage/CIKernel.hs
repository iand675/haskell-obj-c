...
kernelWithFunctionName_fromMetalLibraryData_outputPixelFormat_error name data_ format error_ =
  do
    ...
    withObjCPtr name $ \raw_name ->
      withObjCPtr data_ $ \raw_data_ ->
        withObjCPtr error_ $ \raw_error_ ->
          sendClassMsg cls' (mkSelector "kernelWithFunctionName:fromMetalLibraryData:outputPixelFormat:error:") (retPtr retVoid) [argPtr (castPtr raw_name :: Ptr ()), argPtr (castPtr raw_data_ :: Ptr ()), argCInt (fromIntegral format), argPtr (castPtr raw_error_ :: Ptr ())] >>= retainedObject . castPtr
