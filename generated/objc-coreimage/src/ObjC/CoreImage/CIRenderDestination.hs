...
initWithWidth_height_pixelFormat_commandBuffer_mtlTextureProvider :: IsCIRenderDestination ciRenderDestination => ciRenderDestination -> CULong -> CULong -> MTLPixelFormat -> RawId -> RawId -> IO (Id CIRenderDestination)
initWithWidth_height_pixelFormat_commandBuffer_mtlTextureProvider ciRenderDestination  width height pixelFormat commandBuffer block =
  sendMsg ciRenderDestination (mkSelector "initWithWidth:height:pixelFormat:commandBuffer:mtlTextureProvider:") (retPtr retVoid) [argCULong (fromIntegral width), argCULong (fromIntegral height), argCULong (coerce pixelFormat), argPtr (castPtr (unRawId commandBuffer) :: Ptr ()), argPtr (castPtr (unRawId block) :: Ptr ())] >>= ownedObject . castPtr
...
initWithGLTexture_target_width_height ciRenderDestination  texture target width height =
  sendMsg ciRenderDestination (mkSelector "initWithGLTexture:target:width:height:") (retPtr retVoid) [argCUInt (fromIntegral texture), argCUInt (fromIntegral target), argCULong (fromIntegral width), argCULong (fromIntegral height)] >>= ownedObject . castPtr
...
initWithBitmapData_width_height_bytesPerRow_format ciRenderDestination  data_ width height bytesPerRow format =
  sendMsg ciRenderDestination (mkSelector "initWithBitmapData:width:height:bytesPerRow:format:") (retPtr retVoid) [argPtr data_, argCULong (fromIntegral width), argCULong (fromIntegral height), argCULong (fromIntegral bytesPerRow), argCInt (fromIntegral format)] >>= ownedObject . castPtr
