...
withObjCPtr name $ \raw_name ->
    sendMsg midiumpMutableFunctionBlock (mkSelector "initWithName:direction:firstGroup:totalGroupsSpanned:maxSysEx8Streams:MIDI1Info:UIHint:isEnabled:") (retPtr retVoid) [argPtr (castPtr raw_name :: Ptr ()), argCInt (coerce direction), argCUChar (fromIntegral firstGroup), argCUChar (fromIntegral totalGroupsSpanned), argCUChar (fromIntegral maxSysEx8Streams), argCInt (coerce midI1Info), argCInt (coerce uiHint), argCULong (if isEnabled then 1 else 0)] >>= ownedObject . castPtr
...
withObjCPtr error_ $ \raw_error_ ->
    fmap ((/= 0) :: CULong -> Bool) $ sendMsg midiumpMutableFunctionBlock (mkSelector "reconfigureWithFirstGroup:direction:MIDI1Info:UIHint:error:") retCULong [argCUChar (fromIntegral firstGroup), argCInt (coerce direction), argCInt (coerce midI1Info), argCInt (coerce uiHint), argPtr (castPtr raw_error_ :: Ptr ())]
