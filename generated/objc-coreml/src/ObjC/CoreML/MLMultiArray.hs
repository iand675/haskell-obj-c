...
objectAtIndexedSubscript mlMultiArray  idx =
  sendMsg mlMultiArray (mkSelector "objectAtIndexedSubscript:") (retPtr retVoid) [argCLong (fromIntegral idx)] >>= retainedObject . castPtr
...
withObjCPtr obj_ $ \raw_obj_ ->
    sendMsg mlMultiArray (mkSelector "setObject:atIndexedSubscript:") retVoid [argPtr (castPtr raw_obj_ :: Ptr ()), argCLong (fromIntegral idx)]
...
multiArrayByConcatenatingMultiArrays_alongAxis_dataType multiArrays axis dataType =
  do
    ...
    withObjCPtr multiArrays $ \raw_multiArrays ->
      sendClassMsg cls' (mkSelector "multiArrayByConcatenatingMultiArrays:alongAxis:dataType:") (retPtr retVoid) [argPtr (castPtr raw_multiArrays :: Ptr ()), argCLong (fromIntegral axis), argCLong (coerce dataType)] >>= retainedObject . castPtr
