...
withObjCPtr key $ \raw_key ->
  withObjCPtr error_ $ \raw_error_ ->
      fmap ((/= 0) :: CULong -> Bool) $ sendMsg cwInterface (mkSelector "setWEPKey:flags:index:error:") retCULong [argPtr (castPtr raw_key :: Ptr ()), argCULong (coerce flags), argCLong (fromIntegral index), argPtr (castPtr raw_error_ :: Ptr ())]
...
withObjCPtr ssidData $ \raw_ssidData ->
  withObjCPtr password $ \raw_password ->
    withObjCPtr error_ $ \raw_error_ ->
        fmap ((/= 0) :: CULong -> Bool) $ sendMsg cwInterface (mkSelector "startIBSSModeWithSSID:security:channel:password:error:") retCULong [argPtr (castPtr raw_ssidData :: Ptr ()), argCLong (coerce security), argCULong (fromIntegral channel), argPtr (castPtr raw_password :: Ptr ()), argPtr (castPtr raw_error_ :: Ptr ())]
