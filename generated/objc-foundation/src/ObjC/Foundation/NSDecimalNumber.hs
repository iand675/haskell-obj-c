...
initWithMantissa_exponent_isNegative nsDecimalNumber  mantissa exponent flag =
  sendMsg nsDecimalNumber (mkSelector "initWithMantissa:exponent:isNegative:") (retPtr retVoid) [argCULong (fromIntegral mantissa), argCInt (fromIntegral exponent), argCULong (if flag then 1 else 0)] >>= ownedObject . castPtr
...
decimalNumberWithMantissa_exponent_isNegative mantissa exponent flag =
  do
    ...
    sendClassMsg cls' (mkSelector "decimalNumberWithMantissa:exponent:isNegative:") (retPtr retVoid) [argCULong (fromIntegral mantissa), argCInt (fromIntegral exponent), argCULong (if flag then 1 else 0)] >>= retainedObject . castPtr
...
decimalNumberByRaisingToPower nsDecimalNumber  power =
  sendMsg nsDecimalNumber (mkSelector "decimalNumberByRaisingToPower:") (retPtr retVoid) [argCULong (fromIntegral power)] >>= retainedObject . castPtr
...
decimalNumberByRaisingToPower_withBehavior nsDecimalNumber  power behavior =
  sendMsg nsDecimalNumber (mkSelector "decimalNumberByRaisingToPower:withBehavior:") (retPtr retVoid) [argCULong (fromIntegral power), argPtr (castPtr (unRawId behavior) :: Ptr ())] >>= retainedObject . castPtr
