...
dataWithCapacity aNumItems =
  do
    ...
    sendClassMsg cls' (mkSelector "dataWithCapacity:") (retPtr retVoid) [argCULong (fromIntegral aNumItems)] >>= retainedObject . castPtr
...
dataWithLength length_ =
  do
    cls' <- getRequiredClass "NSMutableData"
    sendClassMsg cls' (mkSelector "dataWithLength:") (retPtr retVoid) [argCULong (fromIntegral length_)] >>= retainedObject . castPtr
...
initWithCapacity nsMutableData  capacity =
  sendMsg nsMutableData (mkSelector "initWithCapacity:") (retPtr retVoid) [argCULong (fromIntegral capacity)] >>= ownedObject . castPtr
...
initWithLength nsMutableData  length_ =
  sendMsg nsMutableData (mkSelector "initWithLength:") (retPtr retVoid) [argCULong (fromIntegral length_)] >>= ownedObject . castPtr
...
appendBytes_length nsMutableData  bytes length_ =
  sendMsg nsMutableData (mkSelector "appendBytes:length:") retVoid [argPtr (unConst bytes), argCULong (fromIntegral length_)]
...
increaseLengthBy nsMutableData  extraLength =
  sendMsg nsMutableData (mkSelector "increaseLengthBy:") retVoid [argCULong (fromIntegral extraLength)]
...
replaceBytesInRange_withBytes_length nsMutableData  range replacementBytes replacementLength =
  sendMsg nsMutableData (mkSelector "replaceBytesInRange:withBytes:length:") retVoid [argNSRange range, argPtr (unConst replacementBytes), argCULong (fromIntegral replacementLength)]
...
setLength nsMutableData  value =
  sendMsg nsMutableData (mkSelector "setLength:") retVoid [argCULong (fromIntegral value)]
