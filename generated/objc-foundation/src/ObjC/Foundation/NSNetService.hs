...
withObjCPtr domain $ \raw_domain ->
  withObjCPtr type_ $ \raw_type_ ->
    withObjCPtr name $ \raw_name ->
        sendMsg nsNetService (mkSelector "initWithDomain:type:name:port:") (retPtr retVoid) [argPtr (castPtr raw_domain :: Ptr ()), argPtr (castPtr raw_type_ :: Ptr ()), argPtr (castPtr raw_name :: Ptr ()), argCInt (fromIntegral port)] >>= ownedObject . castPtr
...
resolveWithTimeout nsNetService  timeout =
  sendMsg nsNetService (mkSelector "resolveWithTimeout:") retVoid [argCDouble (fromIntegral timeout)]
