...
withObjCPtr key $ \raw_key ->
    fmap (RawId . castPtr) $ sendMsg nsObject (mkSelector "valueAtIndex:inPropertyWithKey:") (retPtr retVoid) [argCULong (fromIntegral index), argPtr (castPtr raw_key :: Ptr ())]
...
withObjCPtr key $ \raw_key ->
    sendMsg nsObject (mkSelector "insertValue:atIndex:inPropertyWithKey:") retVoid [argPtr (castPtr (unRawId value) :: Ptr ()), argCULong (fromIntegral index), argPtr (castPtr raw_key :: Ptr ())]
...
withObjCPtr key $ \raw_key ->
    sendMsg nsObject (mkSelector "removeValueAtIndex:fromPropertyWithKey:") retVoid [argCULong (fromIntegral index), argPtr (castPtr raw_key :: Ptr ())]
...
withObjCPtr key $ \raw_key ->
    sendMsg nsObject (mkSelector "replaceValueAtIndex:inPropertyWithKey:withValue:") retVoid [argCULong (fromIntegral index), argPtr (castPtr raw_key :: Ptr ()), argPtr (castPtr (unRawId value) :: Ptr ())]
...
withObjCPtr modes $ \raw_modes ->
    sendMsg nsObject (mkSelector "performSelector:withObject:afterDelay:inModes:") retVoid [argPtr (unSelector aSelector), argPtr (castPtr (unRawId anArgument) :: Ptr ()), argCDouble (fromIntegral delay), argPtr (castPtr raw_modes :: Ptr ())]
...
performSelector_withObject_afterDelay nsObject  aSelector anArgument delay =
  sendMsg nsObject (mkSelector "performSelector:withObject:afterDelay:") retVoid [argPtr (unSelector aSelector), argPtr (castPtr (unRawId anArgument) :: Ptr ()), argCDouble (fromIntegral delay)]
...
withObjCPtr error_ $ \raw_error_ ->
    sendMsg nsObject (mkSelector "attemptRecoveryFromError:optionIndex:delegate:didRecoverSelector:contextInfo:") retVoid [argPtr (castPtr raw_error_ :: Ptr ()), argCULong (fromIntegral recoveryOptionIndex), argPtr (castPtr (unRawId delegate) :: Ptr ()), argPtr (unSelector didRecoverSelector), argPtr contextInfo]
...
withObjCPtr error_ $ \raw_error_ ->
    fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsObject (mkSelector "attemptRecoveryFromError:optionIndex:") retCULong [argPtr (castPtr raw_error_ :: Ptr ()), argCULong (fromIntegral recoveryOptionIndex)]
...
setVersion aVersion =
  do
    cls' <- getRequiredClass "NSObject"
    sendClassMsg cls' (mkSelector "setVersion:") retVoid [argCLong (fromIntegral aVersion)]
