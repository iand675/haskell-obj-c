...
progressWithTotalUnitCount unitCount =
  do
    cls' <- getRequiredClass "NSProgress"
    sendClassMsg cls' (mkSelector "progressWithTotalUnitCount:") (retPtr retVoid) [argCLong (fromIntegral unitCount)] >>= retainedObject . castPtr
...
discreteProgressWithTotalUnitCount unitCount =
  do
    cls' <- getRequiredClass "NSProgress"
    sendClassMsg cls' (mkSelector "discreteProgressWithTotalUnitCount:") (retPtr retVoid) [argCLong (fromIntegral unitCount)] >>= retainedObject . castPtr
...
progressWithTotalUnitCount_parent_pendingUnitCount unitCount parent portionOfParentTotalUnitCount =
  do
    ...
    withObjCPtr parent $ \raw_parent ->
      sendClassMsg cls' (mkSelector "progressWithTotalUnitCount:parent:pendingUnitCount:") (retPtr retVoid) [argCLong (fromIntegral unitCount), argPtr (castPtr raw_parent :: Ptr ()), argCLong (fromIntegral portionOfParentTotalUnitCount)] >>= retainedObject . castPtr
...
becomeCurrentWithPendingUnitCount nsProgress  unitCount =
  sendMsg nsProgress (mkSelector "becomeCurrentWithPendingUnitCount:") retVoid [argCLong (fromIntegral unitCount)]
...
performAsCurrentWithPendingUnitCount_usingBlock nsProgress  unitCount work =
  sendMsg nsProgress (mkSelector "performAsCurrentWithPendingUnitCount:usingBlock:") retVoid [argCLong (fromIntegral unitCount), argPtr (castPtr work :: Ptr ())]
...
withObjCPtr child $ \raw_child ->
    sendMsg nsProgress (mkSelector "addChild:withPendingUnitCount:") retVoid [argPtr (castPtr raw_child :: Ptr ()), argCLong (fromIntegral inUnitCount)]
...
setTotalUnitCount nsProgress  value =
  sendMsg nsProgress (mkSelector "setTotalUnitCount:") retVoid [argCLong (fromIntegral value)]
...
setCompletedUnitCount nsProgress  value =
  sendMsg nsProgress (mkSelector "setCompletedUnitCount:") retVoid [argCLong (fromIntegral value)]
