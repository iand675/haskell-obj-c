...
withObjCPtr modes $ \raw_modes ->
    sendMsg nsRunLoop (mkSelector "performSelector:target:argument:order:modes:") retVoid [argPtr (unSelector aSelector), argPtr (castPtr (unRawId target) :: Ptr ()), argPtr (castPtr (unRawId arg) :: Ptr ()), argCULong (fromIntegral order), argPtr (castPtr raw_modes :: Ptr ())]
