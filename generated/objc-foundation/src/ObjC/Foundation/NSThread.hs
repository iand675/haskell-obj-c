...
sleepForTimeInterval ti =
  do
    cls' <- getRequiredClass "NSThread"
    sendClassMsg cls' (mkSelector "sleepForTimeInterval:") retVoid [argCDouble (fromIntegral ti)]
...
nsThreadSetThreadPriority p =
  do
    cls' <- getRequiredClass "NSThread"
    fmap ((/= 0) :: CULong -> Bool) $ sendClassMsg cls' (mkSelector "setThreadPriority:") retCULong [argCDouble (fromIntegral p)]
...
setThreadPriority nsThread  value =
  sendMsg nsThread (mkSelector "setThreadPriority:") retVoid [argCDouble (fromIntegral value)]
...
setStackSize nsThread  value =
  sendMsg nsThread (mkSelector "setStackSize:") retVoid [argCULong (fromIntegral value)]
