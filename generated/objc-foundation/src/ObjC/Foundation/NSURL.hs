...
getFileSystemRepresentation_maxLength nsurl  buffer maxBufferLength =
  fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsurl (mkSelector "getFileSystemRepresentation:maxLength:") retCULong [argPtr buffer, argCULong (fromIntegral maxBufferLength)]
  ...
  do
    ...
    withObjCPtr bookmarkData $ \raw_bookmarkData ->
      withObjCPtr bookmarkFileURL $ \raw_bookmarkFileURL ->
        withObjCPtr error_ $ \raw_error_ ->
          fmap ((/= 0) :: CULong -> Bool) $ sendClassMsg cls' (mkSelector "writeBookmarkData:toURL:options:error:") retCULong [argPtr (castPtr raw_bookmarkData :: Ptr ()), argPtr (castPtr raw_bookmarkFileURL :: Ptr ()), argCULong (fromIntegral options), argPtr (castPtr raw_error_ :: Ptr ())]
