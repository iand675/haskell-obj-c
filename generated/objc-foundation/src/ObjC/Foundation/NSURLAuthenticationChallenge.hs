  ...
  withObjCPtr credential $ \raw_credential ->
    withObjCPtr response $ \raw_response ->
      withObjCPtr error_ $ \raw_error_ ->
          sendMsg nsurlAuthenticationChallenge (mkSelector "initWithProtectionSpace:proposedCredential:previousFailureCount:failureResponse:error:sender:") (retPtr retVoid) [argPtr (castPtr raw_space :: Ptr ()), argPtr (castPtr raw_credential :: Ptr ()), argCLong (fromIntegral previousFailureCount), argPtr (castPtr raw_response :: Ptr ()), argPtr (castPtr raw_error_ :: Ptr ()), argPtr (castPtr (unRawId sender) :: Ptr ())] >>= ownedObject . castPtr
