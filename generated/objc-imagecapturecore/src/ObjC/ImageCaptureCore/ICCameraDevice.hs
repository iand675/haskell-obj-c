...
withObjCPtr file $ \raw_file ->
    sendMsg icCameraDevice (mkSelector "requestReadDataFromFile:atOffset:length:readDelegate:didReadDataSelector:contextInfo:") retVoid [argPtr (castPtr raw_file :: Ptr ()), argCLong (fromIntegral offset), argCLong (fromIntegral length_), argPtr (castPtr (unRawId readDelegate) :: Ptr ()), argPtr (unSelector selector), argPtr contextInfo]
