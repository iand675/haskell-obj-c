...
withDevice_channelID inDevice inRFCOMMChannelID =
  do
    ...
    withObjCPtr inDevice $ \raw_inDevice ->
      sendClassMsg cls' (mkSelector "withDevice:channelID:") (retPtr retVoid) [argPtr (castPtr raw_inDevice :: Ptr ()), argCUChar (fromIntegral inRFCOMMChannelID)] >>= retainedObject . castPtr
...
withObjCPtr inDevice $ \raw_inDevice ->
    sendMsg ioBluetoothOBEXSession (mkSelector "initWithDevice:channelID:") (retPtr retVoid) [argPtr (castPtr raw_inDevice :: Ptr ()), argCUChar (fromIntegral inChannelID)] >>= ownedObject . castPtr
...
sendDataToTransport_dataLength ioBluetoothOBEXSession  inDataToSend inDataLength =
  sendMsg ioBluetoothOBEXSession (mkSelector "sendDataToTransport:dataLength:") retCInt [argPtr inDataToSend, argCULong (fromIntegral inDataLength)]
