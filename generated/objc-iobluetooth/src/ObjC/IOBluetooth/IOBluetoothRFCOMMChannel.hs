...
registerForChannelOpenNotifications_selector_withChannelID_direction object selector channelID inDirection =
  do
    cls' <- getRequiredClass "IOBluetoothRFCOMMChannel"
    sendClassMsg cls' (mkSelector "registerForChannelOpenNotifications:selector:withChannelID:direction:") (retPtr retVoid) [argPtr (castPtr (unRawId object) :: Ptr ()), argPtr (unSelector selector), argCUChar (fromIntegral channelID), argCInt (coerce inDirection)] >>= retainedObject . castPtr
...
setSerialParameters_dataBits_parity_stopBits ioBluetoothRFCOMMChannel  speed nBits parity bitStop =
  sendMsg ioBluetoothRFCOMMChannel (mkSelector "setSerialParameters:dataBits:parity:stopBits:") retCInt [argCUInt (fromIntegral speed), argCUChar (fromIntegral nBits), argCInt (coerce parity), argCUChar (fromIntegral bitStop)]
