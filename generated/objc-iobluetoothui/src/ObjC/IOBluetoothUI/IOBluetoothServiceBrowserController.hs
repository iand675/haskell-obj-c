...
serviceBrowserController inOptions =
  do
    ...
    sendClassMsg cls' (mkSelector "serviceBrowserController:") (retPtr retVoid) [argCUInt (fromIntegral inOptions)] >>= retainedObject . castPtr
...
browseDevices_options outRecord inOptions =
  do
    ...
    withObjCPtr outRecord $ \raw_outRecord ->
      sendClassMsg cls' (mkSelector "browseDevices:options:") retCInt [argPtr (castPtr raw_outRecord :: Ptr ()), argCUInt (fromIntegral inOptions)]
  ...
  do
    ...
    withObjCPtr outRecord $ \raw_outRecord ->
      withObjCPtr inWindow $ \raw_inWindow ->
        sendClassMsg cls' (mkSelector "browseDevicesAsSheetForWindow:options:window:") retCInt [argPtr (castPtr raw_outRecord :: Ptr ()), argCUInt (fromIntegral inOptions), argPtr (castPtr raw_inWindow :: Ptr ())]
...
setOptions ioBluetoothServiceBrowserController  inOptions =
  sendMsg ioBluetoothServiceBrowserController (mkSelector "setOptions:") retVoid [argCUInt (fromIntegral inOptions)]
