...
initWithLocation_address :: (IsMKMapItem mkMapItem, IsCLLocation location, IsMKAddress address) => mkMapItem -> location -> address -> IO (Id MKMapItem)
initWithLocation_address mkMapItem  location address =
withObjCPtr location $ \raw_location ->
  withObjCPtr address $ \raw_address ->
      sendMsg mkMapItem (mkSelector "initWithLocation:address:") (retPtr retVoid) [argPtr (castPtr raw_location :: Ptr ()), argPtr (castPtr raw_address :: Ptr ())] >>= ownedObject . castPtr
...
-- | @- location@
location :: IsMKMapItem mkMapItem => mkMapItem -> IO (Id CLLocation)
location mkMapItem  =
  sendMsg mkMapItem (mkSelector "location") (retPtr retVoid) [] >>= retainedObject . castPtr
