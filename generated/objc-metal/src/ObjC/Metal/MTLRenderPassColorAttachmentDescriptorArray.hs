...
objectAtIndexedSubscript mtlRenderPassColorAttachmentDescriptorArray  attachmentIndex =
  sendMsg mtlRenderPassColorAttachmentDescriptorArray (mkSelector "objectAtIndexedSubscript:") (retPtr retVoid) [argCULong (fromIntegral attachmentIndex)] >>= retainedObject . castPtr
...
withObjCPtr attachment $ \raw_attachment ->
    sendMsg mtlRenderPassColorAttachmentDescriptorArray (mkSelector "setObject:atIndexedSubscript:") retVoid [argPtr (castPtr raw_attachment :: Ptr ()), argCULong (fromIntegral attachmentIndex)]
