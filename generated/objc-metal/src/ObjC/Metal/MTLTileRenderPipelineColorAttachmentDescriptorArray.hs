...
objectAtIndexedSubscript mtlTileRenderPipelineColorAttachmentDescriptorArray  attachmentIndex =
  sendMsg mtlTileRenderPipelineColorAttachmentDescriptorArray (mkSelector "objectAtIndexedSubscript:") (retPtr retVoid) [argCULong (fromIntegral attachmentIndex)] >>= retainedObject . castPtr
...
withObjCPtr attachment $ \raw_attachment ->
    sendMsg mtlTileRenderPipelineColorAttachmentDescriptorArray (mkSelector "setObject:atIndexedSubscript:") retVoid [argPtr (castPtr raw_attachment :: Ptr ()), argCULong (fromIntegral attachmentIndex)]
