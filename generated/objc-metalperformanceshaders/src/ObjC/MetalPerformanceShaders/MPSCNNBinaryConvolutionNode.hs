...
withObjCPtr sourceNode $ \raw_sourceNode ->
    sendMsg mpscnnBinaryConvolutionNode (mkSelector "initWithSource:weights:scaleValue:type:flags:") (retPtr retVoid) [argPtr (castPtr raw_sourceNode :: Ptr ()), argPtr (castPtr (unRawId weights) :: Ptr ()), argCFloat (fromIntegral scaleValue), argCULong (coerce type_), argCULong (coerce flags)] >>= ownedObject . castPtr
