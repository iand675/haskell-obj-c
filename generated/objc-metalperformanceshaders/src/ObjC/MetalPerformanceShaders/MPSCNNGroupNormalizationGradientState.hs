...
initWithDevice_bufferSize mpscnnGroupNormalizationGradientState  device bufferSize =
  sendMsg mpscnnGroupNormalizationGradientState (mkSelector "initWithDevice:bufferSize:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argCULong (fromIntegral bufferSize)] >>= ownedObject . castPtr
