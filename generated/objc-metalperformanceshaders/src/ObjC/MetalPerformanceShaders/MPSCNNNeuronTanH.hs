...
initWithDevice_a_b mpscnnNeuronTanH  device a b =
  sendMsg mpscnnNeuronTanH (mkSelector "initWithDevice:a:b:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argCFloat (fromIntegral a), argCFloat (fromIntegral b)] >>= ownedObject . castPtr
...
initWithDevice mpscnnNeuronTanH  device =
  sendMsg mpscnnNeuronTanH (mkSelector "initWithDevice:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ())] >>= ownedObject . castPtr
