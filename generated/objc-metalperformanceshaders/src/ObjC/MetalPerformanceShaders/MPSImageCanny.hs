...
initWithDevice_linearToGrayScaleTransform_sigma mpsImageCanny  device transform sigma =
  sendMsg mpsImageCanny (mkSelector "initWithDevice:linearToGrayScaleTransform:sigma:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argPtr (unConst transform), argCFloat (fromIntegral (unConst sigma))] >>= ownedObject . castPtr
...
setHighThreshold mpsImageCanny  value =
  sendMsg mpsImageCanny (mkSelector "setHighThreshold:") retVoid [argCFloat (fromIntegral value)]
...
setLowThreshold mpsImageCanny  value =
  sendMsg mpsImageCanny (mkSelector "setLowThreshold:") retVoid [argCFloat (fromIntegral value)]
