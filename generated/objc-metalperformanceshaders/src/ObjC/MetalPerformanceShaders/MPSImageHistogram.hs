...
encodeToCommandBuffer_sourceTexture_histogram_histogramOffset mpsImageHistogram  commandBuffer source histogram histogramOffset =
  sendMsg mpsImageHistogram (mkSelector "encodeToCommandBuffer:sourceTexture:histogram:histogramOffset:") retVoid [argPtr (castPtr (unRawId commandBuffer) :: Ptr ()), argPtr (castPtr (unRawId source) :: Ptr ()), argPtr (castPtr (unRawId histogram) :: Ptr ()), argCULong (fromIntegral histogramOffset)]
