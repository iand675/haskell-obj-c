...
encodeTransformToCommandBuffer_sourceTexture_histogram_histogramOffset mpsImageHistogramEqualization  commandBuffer source histogram histogramOffset =
  sendMsg mpsImageHistogramEqualization (mkSelector "encodeTransformToCommandBuffer:sourceTexture:histogram:histogramOffset:") retVoid [argPtr (castPtr (unRawId commandBuffer) :: Ptr ()), argPtr (castPtr (unRawId source) :: Ptr ()), argPtr (castPtr (unRawId histogram) :: Ptr ()), argCULong (fromIntegral histogramOffset)]
