...
withObjCPtr descriptor $ \raw_descriptor ->
    sendMsg mpsMatrix (mkSelector "initWithBuffer:offset:descriptor:") (retPtr retVoid) [argPtr (castPtr (unRawId buffer) :: Ptr ()), argCULong (fromIntegral offset), argPtr (castPtr raw_descriptor :: Ptr ())] >>= ownedObject . castPtr
