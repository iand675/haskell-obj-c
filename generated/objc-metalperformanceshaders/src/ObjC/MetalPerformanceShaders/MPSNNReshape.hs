...
withObjCPtr sourceImage $ \raw_sourceImage ->
    sendMsg mpsnnReshape (mkSelector "encodeToCommandBuffer:sourceImage:reshapedWidth:reshapedHeight:reshapedFeatureChannels:") (retPtr retVoid) [argPtr (castPtr (unRawId commandBuffer) :: Ptr ()), argPtr (castPtr raw_sourceImage :: Ptr ()), argCULong (fromIntegral reshapedWidth), argCULong (fromIntegral reshapedHeight), argCULong (fromIntegral reshapedFeatureChannels)] >>= retainedObject . castPtr
...
withObjCPtr sourceImage $ \raw_sourceImage ->
  withObjCPtr outState $ \raw_outState ->
      sendMsg mpsnnReshape (mkSelector "encodeToCommandBuffer:sourceImage:destinationState:destinationStateIsTemporary:reshapedWidth:reshapedHeight:reshapedFeatureChannels:") (retPtr retVoid) [argPtr (castPtr (unRawId commandBuffer) :: Ptr ()), argPtr (castPtr raw_sourceImage :: Ptr ()), argPtr (castPtr raw_outState :: Ptr ()), argCULong (if isTemporary then 1 else 0), argCULong (fromIntegral reshapedWidth), argCULong (fromIntegral reshapedHeight), argCULong (fromIntegral reshapedFeatureChannels)] >>= retainedObject . castPtr
