...
withObjCPtr inputsData $ \raw_inputsData ->
    fmap ((/= 0) :: CULong -> Bool) $ sendMsg mlcInferenceGraph (mkSelector "executeWithInputsData:batchSize:options:completionHandler:") retCULong [argPtr (castPtr raw_inputsData :: Ptr ()), argCULong (fromIntegral batchSize), argCULong (coerce options), argPtr (castPtr completionHandler :: Ptr ())]
...
withObjCPtr inputsData $ \raw_inputsData ->
  withObjCPtr outputsData $ \raw_outputsData ->
      fmap ((/= 0) :: CULong -> Bool) $ sendMsg mlcInferenceGraph (mkSelector "executeWithInputsData:outputsData:batchSize:options:completionHandler:") retCULong [argPtr (castPtr raw_inputsData :: Ptr ()), argPtr (castPtr raw_outputsData :: Ptr ()), argCULong (fromIntegral batchSize), argCULong (coerce options), argPtr (castPtr completionHandler :: Ptr ())]
