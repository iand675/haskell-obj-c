...
layerWithSplitCount_dimension splitCount dimension =
  do
    ...
    sendClassMsg cls' (mkSelector "layerWithSplitCount:dimension:") (retPtr retVoid) [argCULong (fromIntegral splitCount), argCULong (fromIntegral dimension)] >>= retainedObject . castPtr
...
layerWithSplitSectionLengths_dimension splitSectionLengths dimension =
  do
    ...
    withObjCPtr splitSectionLengths $ \raw_splitSectionLengths ->
      sendClassMsg cls' (mkSelector "layerWithSplitSectionLengths:dimension:") (retPtr retVoid) [argPtr (castPtr raw_splitSectionLengths :: Ptr ()), argCULong (fromIntegral dimension)] >>= retainedObject . castPtr
