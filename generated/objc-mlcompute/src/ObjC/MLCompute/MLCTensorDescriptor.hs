...
descriptorWithWidth_height_featureChannelCount_batchSize width height featureChannels batchSize =
  do
    cls' <- getRequiredClass "MLCTensorDescriptor"
    sendClassMsg cls' (mkSelector "descriptorWithWidth:height:featureChannelCount:batchSize:") (retPtr retVoid) [argCULong (fromIntegral width), argCULong (fromIntegral height), argCULong (fromIntegral featureChannels), argCULong (fromIntegral batchSize)] >>= retainedObject . castPtr
...
descriptorWithWidth_height_featureChannelCount_batchSize_dataType width height featureChannelCount batchSize dataType =
  do
    cls' <- getRequiredClass "MLCTensorDescriptor"
    sendClassMsg cls' (mkSelector "descriptorWithWidth:height:featureChannelCount:batchSize:dataType:") (retPtr retVoid) [argCULong (fromIntegral width), argCULong (fromIntegral height), argCULong (fromIntegral featureChannelCount), argCULong (fromIntegral batchSize), argCInt (coerce dataType)] >>= retainedObject . castPtr
...
convolutionWeightsDescriptorWithWidth_height_inputFeatureChannelCount_outputFeatureChannelCount_dataType width height inputFeatureChannelCount outputFeatureChannelCount dataType =
  do
    cls' <- getRequiredClass "MLCTensorDescriptor"
    sendClassMsg cls' (mkSelector "convolutionWeightsDescriptorWithWidth:height:inputFeatureChannelCount:outputFeatureChannelCount:dataType:") (retPtr retVoid) [argCULong (fromIntegral width), argCULong (fromIntegral height), argCULong (fromIntegral inputFeatureChannelCount), argCULong (fromIntegral outputFeatureChannelCount), argCInt (coerce dataType)] >>= retainedObject . castPtr
...
convolutionWeightsDescriptorWithInputFeatureChannelCount_outputFeatureChannelCount_dataType inputFeatureChannelCount outputFeatureChannelCount dataType =
  do
    cls' <- getRequiredClass "MLCTensorDescriptor"
    sendClassMsg cls' (mkSelector "convolutionWeightsDescriptorWithInputFeatureChannelCount:outputFeatureChannelCount:dataType:") (retPtr retVoid) [argCULong (fromIntegral inputFeatureChannelCount), argCULong (fromIntegral outputFeatureChannelCount), argCInt (coerce dataType)] >>= retainedObject . castPtr
...
convolutionBiasesDescriptorWithFeatureChannelCount_dataType featureChannelCount dataType =
  do
    cls' <- getRequiredClass "MLCTensorDescriptor"
    sendClassMsg cls' (mkSelector "convolutionBiasesDescriptorWithFeatureChannelCount:dataType:") (retPtr retVoid) [argCULong (fromIntegral featureChannelCount), argCInt (coerce dataType)] >>= retainedObject . castPtr
