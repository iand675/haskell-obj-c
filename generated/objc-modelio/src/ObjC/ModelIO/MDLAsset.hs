...
objectAtIndexedSubscript mdlAsset  index =
  sendMsg mdlAsset (mkSelector "objectAtIndexedSubscript:") (retPtr retVoid) [argCULong (fromIntegral index)] >>= retainedObject . castPtr
...
objectAtIndex mdlAsset  index =
  sendMsg mdlAsset (mkSelector "objectAtIndex:") (retPtr retVoid) [argCULong (fromIntegral index)] >>= retainedObject . castPtr
...
placeLightProbesWithDensity_heuristic_usingIrradianceDataSource value type_ dataSource =
  do
    cls' <- getRequiredClass "MDLAsset"
    sendClassMsg cls' (mkSelector "placeLightProbesWithDensity:heuristic:usingIrradianceDataSource:") (retPtr retVoid) [argCFloat (fromIntegral value), argCLong (coerce type_), argPtr (castPtr (unRawId dataSource) :: Ptr ())] >>= retainedObject . castPtr
...
setFrameInterval mdlAsset  value =
  sendMsg mdlAsset (mkSelector "setFrameInterval:") retVoid [argCDouble (fromIntegral value)]
...
setStartTime mdlAsset  value =
  sendMsg mdlAsset (mkSelector "setStartTime:") retVoid [argCDouble (fromIntegral value)]
...
setEndTime mdlAsset  value =
  sendMsg mdlAsset (mkSelector "setEndTime:") retVoid [argCDouble (fromIntegral value)]
