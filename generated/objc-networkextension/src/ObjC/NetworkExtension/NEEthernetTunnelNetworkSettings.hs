...
withObjCPtr address $ \raw_address ->
  withObjCPtr ethernetAddress $ \raw_ethernetAddress ->
      sendMsg neEthernetTunnelNetworkSettings (mkSelector "initWithTunnelRemoteAddress:ethernetAddress:mtu:") (retPtr retVoid) [argPtr (castPtr raw_address :: Ptr ()), argPtr (castPtr raw_ethernetAddress :: Ptr ()), argCLong (fromIntegral mtu)] >>= ownedObject . castPtr
