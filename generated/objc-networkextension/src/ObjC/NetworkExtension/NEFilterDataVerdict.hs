...
dataVerdictWithPassBytes_peekBytes passBytes peekBytes =
  do
    cls' <- getRequiredClass "NEFilterDataVerdict"
    sendClassMsg cls' (mkSelector "dataVerdictWithPassBytes:peekBytes:") (retPtr retVoid) [argCULong (fromIntegral passBytes), argCULong (fromIntegral peekBytes)] >>= retainedObject . castPtr
