...
viewControllerAtRow_makeIfNecessary ncWidgetListViewController  row makeIfNecesary =
  sendMsg ncWidgetListViewController (mkSelector "viewControllerAtRow:makeIfNecessary:") (retPtr retVoid) [argCULong (fromIntegral row), argCULong (if makeIfNecesary then 1 else 0)] >>= retainedObject . castPtr
...
setMinimumVisibleRowCount ncWidgetListViewController  value =
  sendMsg ncWidgetListViewController (mkSelector "setMinimumVisibleRowCount:") retVoid [argCULong (fromIntegral value)]
