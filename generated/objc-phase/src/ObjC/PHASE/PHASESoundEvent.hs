...
seekToTime_completion phaseSoundEvent  time handler =
  sendMsg phaseSoundEvent (mkSelector "seekToTime:completion:") retVoid [argCDouble (fromIntegral time), argPtr (castPtr handler :: Ptr ())]
...
withObjCPtr engineTime $ \raw_engineTime ->
    sendMsg phaseSoundEvent (mkSelector "seekToTime:resumeAtEngineTime:completion:") retVoid [argCDouble (fromIntegral time), argPtr (castPtr raw_engineTime :: Ptr ()), argPtr (castPtr handler :: Ptr ())]
