...
initWithProcessIdentifier sbApplication  pid =
  sendMsg sbApplication (mkSelector "initWithProcessIdentifier:") (retPtr retVoid) [argCInt (fromIntegral pid)] >>= ownedObject . castPtr
...
applicationWithProcessIdentifier pid =
  do
    cls' <- getRequiredClass "SBApplication"
    sendClassMsg cls' (mkSelector "applicationWithProcessIdentifier:") (retPtr retVoid) [argCInt (fromIntegral pid)] >>= retainedObject . castPtr
...
launchFlags :: IsSBApplication sbApplication => sbApplication -> IO LSLaunchFlags
launchFlags sbApplication  =
  fmap (coerce :: CUInt -> LSLaunchFlags) $ sendMsg sbApplication (mkSelector "launchFlags") retCUInt []
...
setLaunchFlags :: IsSBApplication sbApplication => sbApplication -> LSLaunchFlags -> IO ()
setLaunchFlags sbApplication  value =
  sendMsg sbApplication (mkSelector "setLaunchFlags:") retVoid [argCUInt (coerce value)]
...
setSendMode sbApplication  value =
  sendMsg sbApplication (mkSelector "setSendMode:") retVoid [argCInt (fromIntegral value)]
...
setTimeout sbApplication  value =
  sendMsg sbApplication (mkSelector "setTimeout:") retVoid [argCLong (fromIntegral value)]
