...
defaultCriticalSoundWithAudioVolume volume =
  do
    ...
    sendClassMsg cls' (mkSelector "defaultCriticalSoundWithAudioVolume:") (retPtr retVoid) [argCFloat (fromIntegral volume)] >>= retainedObject . castPtr
...
criticalSoundNamed_withAudioVolume name volume =
  do
    cls' <- getRequiredClass "UNNotificationSound"
    withObjCPtr name $ \raw_name ->
      sendClassMsg cls' (mkSelector "criticalSoundNamed:withAudioVolume:") (retPtr retVoid) [argPtr (castPtr raw_name :: Ptr ()), argCFloat (fromIntegral volume)] >>= retainedObject . castPtr
